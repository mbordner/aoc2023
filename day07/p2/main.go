...
const (
	handHighCard    typeHand = iota // 5 unique 1,1,1,1,1 -> one pair
	handOnePair                     // 4 unique 2,1,1,1 -> three of kind
	handTwoPair                     // 3 unique 2,2,1 (4) -> full house
...
func highestValueCard(cc cardCounts) string {
	c := "J"
	l := 1
	for k, v := range cc {
		if k != "J" {
...
func upgradeJokers(cc cardCounts) {
	if jl, e := cc["J"]; e {
		c := highestValueCard(cc)
		if c != "J" {
			delete(cc, "J")
			cc[c] += jl
		}
